...
are enough observations per feature, otherwise `"gaussian"`. See
...
# phi0 is auto-computed as mean(pred_nestcv_glmnet(fit, x))
...
(default `20`) observations per predictor, otherwise `"gaussian"` (with a
warning, since multivariate normality is assumed but not checked). Adjust the
threshold or force a specific approach as needed:
