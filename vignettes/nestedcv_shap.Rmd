...
are enough observations per feature, otherwise `"independence"`. See
...
automatically computes the baseline (`phi0`) as the mean predicted value over
the data, and returns the `shapr` object directly. This can be passed
straight into the plotting functions below, or used with `shapr`'s own
`print()`/`plot()` methods.
...
sh <- nestcv.explain(fit_gbm, x_explain = x, predict_model = pred_train)
plot_shap_beeswarm(sh, x, size = 1)
```
...
```{r, eval = shapr_eval, fig.width = 9, fig.height = 3.5}
library(ggplot2)
data("iris")
dat <- iris
y <- dat$Species
...
sh1 <- nestcv.explain(fit_iris, x_explain = x,
                      predict_model = pred_nestcv_glmnet_class(1))
sh2 <- nestcv.explain(fit_iris, x_explain = x,
                      predict_model = pred_nestcv_glmnet_class(2))
sh3 <- nestcv.explain(fit_iris, x_explain = x,
...
By default `nestcv.explain()` computes `phi0` automatically as the mean
prediction over the data supplied in `x_explain`. You can override this with
any reference value, for example the mean of the response from a separate
training set:

```{r, eval=FALSE}
sh <- nestcv.explain(fit, x_explain = x, predict_model = pred_nestcv_glmnet,
                     phi0 = mean(y_train))
```

`nestcv.explain()` also auto-selects an `approach` for `shapr::explain()` based
on `x_train`'s column types: `"categorical"` if all predictors are
factor/character, `"ctree"` for a mix of numeric and factor/character
predictors, and for all-numeric predictors, `"empirical"` (accounts for
feature correlation) provided there are at least `min_obs_per_feature`
(default `20`) observations per predictor, otherwise `"independence"`. Adjust
the threshold or force a specific approach as needed:
...
in `x` it is much better to pass only the subset of predictors in the final
