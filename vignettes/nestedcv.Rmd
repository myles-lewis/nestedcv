...
By default `nestedcv` stratifies folds based on the outcome variable using
caret's `createFolds()` function. Users can control CV folds directly by
providing their own lists of fold indices. Outer CV fold indices can simply be
provided via the argument `outer_folds` to `nestcv.glmnet()` or
