...
# single fit
fit <- nestcv.glmnet(y, x, family = "gaussian", alphaSet = 1, cv.cores = 2)

# repeated nested CV
res <- repeatcv(n = 5, nestcv.glmnet(y, x, family = "gaussian", alphaSet = 1,
...
```{r eval=FALSE}
# using a pipe from the magrittr package
`%|>%` <- magrittr::pipe_nested

res <- nestcv.glmnet(y, x, family = "gaussian", alphaSet = 1, cv.cores = 2) %|>%
  repeatcv(5)
```
